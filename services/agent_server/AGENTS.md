...
uv run python main.py serve --host 0.0.0.0 --port 8888
...
- **插件发现**: 始终扫描内置 `builtins/` 和固定外部目录 `/plugins`
...
- 配置集中在 `settings.py`，只通过 `AGENT_SERVER_*` 环境变量覆盖运行参数
- `auth.db_url` 为空字符串时**跳过认证**；开发用 SQLite，生产用 PostgreSQL
- `/chat` 的 `provider` 必须由请求显式传入，不设置默认 Provider
- 固定外部插件目录中的每个一级子目录必须包含 `__init__.py` 和一个 `Plugin` 子类
